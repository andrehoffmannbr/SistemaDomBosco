...
const FUNC_FIELD_IDS = {
  ...
  unit:    ['func-unit', 'unidade', 'employee-unit'],
  password:['func-password', 'senha', 'employee-password'],
};
...
function firstExistingId(ids) {
  for (const id of ids) {
    ...
    if (byName) return byName.id || id;
  }
  return null;
}

function readField(ids, { numeric = false } = {}) {
  const id = firstExistingId(ids);
  if (!id) return numeric ? null : '';
  return numeric ? ($num(id)) : ($val(id));
}

function collectTabAccess(container) {
  // Coleta todas as permissões do container
  const out = {};
  const root = container || document;
  const selects = root.querySelectorAll('select[data-tabkey]');
  selects.forEach(sel => {
    const key = sel.getAttribute('data-tabkey');
    const raw = (sel.value || '').toLowerCase();
    // normaliza 'ver'/'editar'/'sem acesso' e variantes
    const map = {
      'ver': 'view', 'view': 'view',
      'editar':'edit','edit':'edit',
      'sem acesso':'none','none':'none','': 'none'
    };
...
export async function addNewFuncionario(ev) {
  try {
    ...
    // Guardas de permissão (mantêm lógica atual)
    const role = getCurrentUser?.()?.role ?? null;
    if (!isUserRoleIn?.(role, ['director','admin'])) {
      showNotification?.('Sem permissão para criar usuários. Peça acesso ao diretor/admin.', 'error');
      return;
    }
    if (!checkTabAccess?.('funcionarios','create') && !checkTabAccess?.('users','create')) {
      showNotification?.('Sem permissão para criar usuários. Peça acesso ao diretor/admin.', 'error');
      return;
    }
    ...
    const cpf   = readField(FUNC_FIELD_IDS.cpf);
    ...
    if (!name || !email || !password) {
      showNotification?.('Nome, e-mail e senha são obrigatórios.', 'warning');
      return;
    }
