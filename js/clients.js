...
export function renderClientList(filter = '', activityFilter = 'all', professionalFilter = 'all', unitFilter = 'all') {
    ...
    if (isUserRoleIn(currentUser?.role, PROFESSIONAL_ROLES) && !isUserRoleIn(currentUser?.role, ALL_ADMIN_VIEW_CLIENTS_AND_EMPLOYEES)) {
...
export function showClientDetails(clientId) {
    // Evita quebras quando modal/elementos não existem na aba atual
    ...
    if (client.type === 'adult') {
        _txt('modal-email-cliente', client.email || 'N/A');
        _txt('modal-telefone-cliente', client.phone || 'N/A');
        _txt('modal-cpf-cliente', client.cpf || 'N/A');
        _txt('modal-responsavel-cliente', client.contatoEmergencia || 'N/A');
    } else {
        _txt('modal-email-cliente', 'N/A (Menor de idade)');
        _txt('modal-telefone-cliente', `Pais: ${client.telefonePai || 'N/A'} / ${client.telefoneMae || 'N/A'}`);
        _txt('modal-cpf-cliente', 'N/A (Menor de idade)');
        _txt('modal-responsavel-cliente', `${client.nomePai || ''} / ${client.nomeMae || ''}`);
    }
    
    _txt('modal-data-nascimento', client.birthDate ? new Date(client.birthDate).toLocaleDateString('pt-BR', {timeZone: 'UTC'}) : 'N/A');
    _txt('modal-cep-cliente', client.cep || 'N/A');
    _txt('modal-logradouro-cliente', client.address || 'N/A');
    _txt('modal-numero-cliente', client.number || 'S/N');
    _txt('modal-bairro-cliente', client.neighborhood || '');
    _txt('modal-cidade-estado-cliente', `${client.city || ''} / ${client.state || ''}`);
    _txt('modal-observacoes-cliente', client.observations || 'Nenhuma observação.');
    ...
    _txt('modal-unidade-cliente', unitMap[client.unit] || 'N/A');
    ...
    if (deleteClientButton) {
        if (checkTabAccess('historico', 'edit') && isUserRoleIn(getCurrentUser()?.role, DIRECTOR_ONLY)) { // Only Director can delete
...
function renderAppointmentHistory(appointments) {
    ...
    if (!historicoContainer) return; // guard: não renderiza se não estiver nesta aba
...
export async function addClientDocument(clientId, doc) {
    ...
    if (!(isUserRoleIn(getCurrentUser()?.role, ALL_ADMIN_VIEW_CLIENTS_AND_EMPLOYEES) || isUserRoleIn(getCurrentUser()?.role, PROFESSIONAL_ROLES))) {
...
export function renderMeusPacientes(filter = '') {
    const currentUser = getCurrentUser();
    if (!isUserRoleIn(currentUser?.role, PROFESSIONAL_ROLES)) return; // Ensure only professionals can access this function
