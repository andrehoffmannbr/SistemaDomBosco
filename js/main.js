...
// ─────────────────────────────
// Helpers globais idempotentes
// ─────────────────────────────
globalThis.el ??= (id) => document.getElementById(id);
globalThis.setTextById ??= (id, v) => { const n = el(id); if (n) n.textContent = (v ?? '') + ''; };
globalThis.setValueById ??= (id, v) => { const n = el(id); if (n) n.value = v ?? ''; };
globalThis.safeArray ??= (arr) => Array.isArray(arr) ? arr : [];
globalThis.safeNum ??= (v) => { const n = Number(v); return Number.isFinite(n) ? n : 0; };
globalThis.fmtMoney ??= (v) => safeNum(v).toLocaleString('pt-BR',{ minimumFractionDigits:2, maximumFractionDigits:2 });
...
// Não deixe 1 pane derrubar o app
...
async function rehydrateCurrentUserFromSession() {
  try {
    const session = await getSession();
    const uid = session?.user?.id;
    if (!uid) return false;
    // Evita trabalho se já está populado
    const cur = (() => { try { return JSON.parse(localStorage.getItem('currentUser')||'null'); } catch { return null; }})();
    if (cur?.id === uid) return true;
    // Busca profile e grava
    ...
    console.warn('[rehydrateCurrentUserFromSession] falhou:', e);
    return false;
  }
}

// (removido bloco duplicado de helpers — mantemos o primeiro bloco com toLocaleString)
...
// (removido bloco duplicado — helpers já definidos no topo via globalThis.*)
...
async function bootApp() {
    ...
    const session = await getSession();
    if (session?.user?.id) {
        // 1) Reidrata currentUser para que checkTabAccess/governança funcionem após reload
        await rehydrateCurrentUserFromSession();
        // 2) Hidrata os dados
        try { await hydrateAll(); } catch(e) { console.error('Falha na hidratação inicial:', e); }
    }
    setupEventListeners();
    setupFormHandlers();
    setupMobileGestures(); 
...
async function initializeApp() {
    ...
    // Garante que currentUser exista (após reload + sessão)
    const ok = getCurrentUser() || await rehydrateCurrentUserFromSession();
    // Renderiza o menu com permissões adequadas
    renderMenu();
    
    const currentUser = getCurrentUser();
    console.log('[init] currentUser:', currentUser?.role, currentUser?.tabAccess ? 'tabAccess(ok)' : 'tabAccess(vazio)');
    ...
    safeInit('client-list', () => { if (onPage('client-list-container')) renderClientList(); });
    safeInit('meus-pacientes', () => { if (onPage('client-list-container')) renderMeusPacientes(); });
    if (checkTabAccess('financeiro', 'view')) {
        safeInit('finance-report', () => {
            if (onPage('financial-summary-grid')) {
...
function renderMenu() {
  ...
  safeArray(tabs).forEach(([section, sel]) => {
...
function setupEventListeners() {
    ...
    document.addEventListener('click', (event) => {
        ...
        const dropdown = document.getElementById('notification-dropdown');
        if (!dropdown || !bellWrapper) return; // guard: elementos podem não existir nesta view
    ...
    Array.from(document.querySelectorAll('.tab-button')).forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (!checkTabAccess(tabId, 'view')) {
                const label = button.querySelector('span')?.textContent || tabId;
                showNotification(`Você não tem permissão para acessar a aba "${label}".`, 'error');
            ...
            if (tabId === 'historico') {
                ...
                updateStockSummary().catch(() => { /* não travar a UI */ });
            ...
            <div class="form-group form-group-large">
                <select class="material-item" required>
                    ...
                    ${db.stockItems.map(item => {
                        return `<option value="${item.id}">${item.name} - R$ ${fmtMoney(item.unitValue).replace('.', ',')} / ${fmtInt(item.quantity)} unidades disponíveis</option>`;
                    }).join('')}
                </select>
            </div>
...
function softRefreshData() {
    // 1. Don't refresh if user is typing or a modal is open
    ...
    if (isTyping || isModalOpen) {
        // console.log('Skipping soft refresh due to user activity.');
        return;
    }

    // 2. Find the active tab and call its render function
    const activeTab = document.querySelector('.tab-content.active');
    if (!activeTab) return;

    const tabId = activeTab.id.replace('tab-', '');
    
    // This is similar to the tab switching logic
    switch (tabId) {
        case 'historico':
            const searchTerm = document.getElementById('search-cliente').value;
            const activityFilter = document.getElementById('client-activity-filter-select').value;
        ...
        case 'documentos':
            renderGeneralDocuments();
            break;
        // No action for 'cadastro' as it's a form
        case 'cadastro':
            break;
    }
    
    // Also update global components like notifications
    checkNotifications();
