...
export const checkTabAccess = (globalThis.checkTabAccess ??=
  (tab, action = 'view', currentUser) => {
    ...
    // Fallback seguro: se não houver config para a aba, 'view' = allow, 'edit' = deny
    if (!access) return action === 'view';
    if (action === 'edit') {
...
// Helpers opcionais para reduzir erro de uso:
export const roleOf = () => {
  try { return (globalThis.getCurrentUser?.() || JSON.parse(localStorage.getItem('currentUser')||'null'))?.role ?? null; }
  catch { return null; }
};
export const hasRoleIn = (set) => isUserRoleIn(roleOf(), set);
